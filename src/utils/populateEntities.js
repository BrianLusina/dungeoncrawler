 ...
 * @notes: Entity creator
 */

export default function populateEntities(gameMap) {
    ...
    var qualifying = weaponTypes.filter(function (weapon) {
        return weapon.damage < level * 10 + 10;
    }).filter(function (weapon) {
        return weapon.damage > level * 10 - 10;
    });
    for (var i = 0; i < 3; i++) {
        var weapon = Object.assign({}, qualifying[_.random(0, qualifying.length - 1)]);
        weapon.type = 'weapon';
        weapons.push(weapon);
    }
    ...
    var playerPosition = [];
    [potions, enemies, weapons, exits, players, bosses].forEach(function (entities) {
        while (entities.length) {
            var x = Math.floor(Math.random() * c.GRID_WIDTH);
            var y = Math.floor(Math.random() * c.GRID_HEIGHT);
